...
for iy in range(unit_cell_y):
	for ix in range(unit_cell_x):
		create_cap1(
			...
			oEditor=oEditor,
			oDesign=oDesign,
			name_cap="layer1_cap"+str(layer1_cap_index),
			position_grid_x=ix,
			position_grid_y=iy,
...
## Create Layer 3 & 5
import csv
file_path = 'D:/Master_NSYSU/CodeRepository_ThisPeriod/zhar_antenna_design/py_helper/2026_1_11_metasurfaceBuilder/antenna_design_full18x18_merged.csv'
...
for i, row in enumerate(csv_data):
	create_cap2(
		...
		oEditor=oEditor,
		oDesign=oDesign,
		name_cap="layer3_cap"+str(layer3_cap_index),
		position_grid_x=row['x'],
		position_grid_y=row['y'],
		size_patch=row['top_length_2_mm']
	)
	layer3_cap_index = layer3_cap_index+1
	create_cap3(
		oProject=oProject,
		oEditor=oEditor,
		oDesign=oDesign,
		name_cap="layer5_cap"+str(layer5_cap3_index),
		position_grid_x=row['x'],
		position_grid_y=row['y'],
		size_patch=row['top_length_3_mm']
	)
	layer5_cap3_index = layer5_cap3_index+1
    # print(f"Row {i+1}: {row}")
